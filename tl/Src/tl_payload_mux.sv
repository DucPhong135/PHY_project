...
#(
  ...
  output logic [1:0]              wdata_consumed_dw_o,  // â†? NEW: How many DWs consumed
    ...
    case (state)
      HDR_BEAT: begin
        if (is_3dw_header && (total_data_dw != 12'd0)) begin
          if(hdr_reg[6] == 1'b1 && hdr_reg[4:0] == 5'b00000) begin // Write request with 3DW header
            ...
            wdata_consumed_dw_o   = 2'd1;  // â†? Only 1 DW consumed
          end
            ...
            wdata_consumed_dw_o   = 2'd0;  // â†? No data consumed yet
          end
        end
        else begin
          // 4DW header
      ...
      DATA_BEAT: begin
        ...
        if(is_3dw_header) begin
          ...
          wdata_ready_o       = tx_pkt_ready_i;
          wdata_consumed_dw_o = dw_this_beat[1:0];  // â†? 1-4 DWs consumed
        end
      end
