...
class tl_tb extends uvm_env;
    ...
    bit cpl_scoreboard_enabled = 1'b1;
    ...
    cpl_scoreboard cpl_scoreboard_inst;
    ...
    function void build_phase(uvm_phase phase);
        ...
        if (!uvm_config_db#(bit)::get(this, "", "tx_scoreboard_enabled", tx_scoreboard_enabled)) begin
            tx_scoreboard_enabled = 1'b0;  // Default: enabled
        ...
        if (!uvm_config_db#(bit)::get(this, "", "rx_scoreboard_enabled", rx_scoreboard_enabled)) begin
            rx_scoreboard_enabled = 1'b0;  // Default: enabled
            `uvm_info("TL_TB", "rx_scoreboard_enabled not set in config_db, defaulting to 1", UVM_LOW);
        end
        if (!uvm_config_db#(bit)::get(this, "", "cpl_scoreboard_enabled", cpl_scoreboard_enabled)) begin
            cpl_scoreboard_enabled = 1'b0;  // Default: enabled
            `uvm_info("TL_TB", "cpl_scoreboard_enabled not set in config_db, defaulting to 1", UVM_LOW);
        end
        ...
        if (cpl_scoreboard_enabled) begin
            cpl_scoreboard_inst = cpl_scoreboard::type_id::create("cpl_scoreboard_inst", this);
        end
    ...
    function void connect_phase(uvm_phase phase);
        ...
        if (cpl_scoreboard_enabled) begin
            // Connect user monitor to completion scoreboard
            user_env.user_agent.user_monitor.cpl_ap.connect(cpl_scoreboard_inst.user_cpl_ap);
            dll_env.dll_agent.dll_monitor.rx_monitor_ap.connect(cpl_scoreboard_inst.dll_cpl_ap);
        end
