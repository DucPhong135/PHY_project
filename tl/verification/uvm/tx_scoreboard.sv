...
class tx_scoreboard extends uvm_scoreboard;
  ...
  tl_user_seq_item expected_user[$]; // Queue to store expected user transactions
  tl_tlp_seq_item mismatch_tlp[$]; // Queue to store expected TLPs
  ...
  function void write_user(tl_user_seq_item user);
    // Compare received TLP with expected behavior
  ...
  function bit compare_transactions(tl_user_seq_item user, tl_tlp_seq_item tlp);
    // Implement comparison logic between user transaction and TLP
    ...
    // Memory transaction - check 64-bit address (DW-aligned, [1:0] reserved)
    ...
    if(user.trans_type == CMD_CFG) begin
        ...
        // For memory transactions, calculate expected BE based on addr and length
    ...
    // Check payload size matches
        ...
        // Read request should have no payload
